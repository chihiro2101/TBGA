...
class Summerizer(object):
    ...
    def selection(self, population):
        ...
        # max_sent = int(0.25*len(self.sentences))
        max_sent = 4
        ...
        # while cpop < (population_size-1)/2:
        check_time = time.time()
        while cpop <= population_size and (time.time() - check_time) < 900:
            population[0] = sorted(population[0], key=lambda x: x[2], reverse=True)
            parent_1 = None

            check_time_1 = time.time()
            while parent_1 == None and (time.time() - check_time_1) < 600:
                parent_1 = self.rank_select(population[0])
            parent_2 = None

            check_time_2 = time.time()
            while parent_2 == None and (time.time() - check_time_2) < 600:
